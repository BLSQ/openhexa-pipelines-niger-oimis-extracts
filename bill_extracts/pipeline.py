...
@pipeline(code="bill-extracts", name="bill_extracts")
@parameter(
    "date_year_from",
...
def bill_extracts(run_auto:bool, date_year_from:str, date_month_from:str, date_year_to:str, date_month_to:str):
    """
    In this pipeline we call a notebook that executes the graph queries
    
    """

    # Setup variables
    notebook_name = "graphQL_bills_Niger"  
    notebook_path = f"{workspace.files_path}/pipelines/Bill_extracts/" 
    out_notebook_path = f"{workspace.files_path}/pipelines/Bill_extracts/papermill_outputs"
    

    if run_auto:
        dateBill_Gte, dateBill_Lte = get_previous_month_dates()
        current_run.log_info(f"Automatic run from: {dateBill_Gte} to: {dateBill_Lte}")    

    else:
        # Date input checks - FROM
        if date_year_from is None and date_month_from is None:
            dateBill_Gte = None # then we dont filter .. download everything
        else:
            ...
            dateBill_Gte = f"{date_year_from}-{date_month_from:02d}-01"
            current_run.log_info(f"Retrieve data from: {dateBill_Gte}")
        ...
        if date_year_to is None or date_month_to is None:
            ...
            dateBill_Lte = get_last_day_of_month(f"{date_year_to}-{date_month_to:02d}")          
            current_run.log_info(f"Retrieve data to: {dateBill_Lte}")
            
    parameters = {
        'dateBill_Gte': dateBill_Gte,
        'dateBill_Lte': dateBill_Lte,
    }
