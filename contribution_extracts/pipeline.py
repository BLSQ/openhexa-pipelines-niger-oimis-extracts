...
def contribution_extracts(run_auto:bool, date_year_from:str, date_month_from:str, date_year_to:str, date_month_to:str):
    """
    ...
    if run_auto:
        enrollDate_Gte, enrollDate_Lte = get_previous_month_dates()
        current_run.log_info(f"Automatic run from: {enrollDate_Gte} to: {enrollDate_Lte}")    

    else:
        # Date input checks - FROM
        if date_year_from is None and date_month_from is None:
            enrollDate_Gte = None # then we dont filter .. download everything
        else:
            ...
            enrollDate_Gte = f"{date_year_from}-{date_month_from:02d}-01"
            current_run.log_info(f"Retrieve data from: {enrollDate_Gte}")
        ...
        if date_year_to is None or date_month_to is None:
            ...
            enrollDate_Lte = get_last_day_of_month(f"{date_year_to}-{date_month_to:02d}")           
            current_run.log_info(f"Retrieve data to: {enrollDate_Lte}")
            
    # Set parameters
    parameters = {        
        'enrollDate_Gte': enrollDate_Gte,
        'enrollDate_Lte': enrollDate_Lte,
    }
